 ...
 * 删除理由:折扣只对"值平均正确、噪声偏大"有意义;这两类行属于
 * "值可能整个是错的",对它们打折既救不了错值、又白白削弱对的值。
 * 单边行的上游已由逐行种子搜索修正(丢线返回上一行边线而非画面边界),
 * 补线行则已有 cross_bottom_track_ok() 这道闸决定要不要补。
 * 不要再加回来——要处理不确定度就修上游或加闸,不要加系数。 */
