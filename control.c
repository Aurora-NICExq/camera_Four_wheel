/* control.c - steer PD + duty cap (rows/curv/steer LUT + exit gate) */
#include <stdint.h>
#include "config.h"
#include "image.h"
#include "control.h"
...
void control_update(const track_info_t *ti, control_out_t *out)
{
    ...
    {
        int16_t err_abs = (error >= 0) ? error : (int16_t)(-error);
        if (err_abs <= EXIT_ERR_MAX && ti->valid_rows >= EXIT_ROWS_MIN &&
            curv_abs <= EXIT_CURV_MAX && ti->both_lost_rows <= EXIT_MAX_BOTH_LOST)
        {
        ...
        if (target > g_duty_now && g_exit_cnt < EXIT_CONFIRM_FRAMES)
        {
