/*********************************************************************************************************************
 * pins.h — 全部引脚/通道分配（唯一存放处）。已按 TC264_引脚分配.md 实测填写；PINS_CONFIGURED 已定义。
 * 通道枚举名见 libraries/zf_driver/zf_driver_pwm.h（PWM）与 zf_driver_gpio.h（GPIO）。
 * 约束：舵机(50Hz)与电机(~17kHz)频率不同，必须位于不同 ATOM 模块（舵机 ATOM1，电机 ATOM0）。
 ********************************************************************************************************************/
#ifndef PINS_H
#define PINS_H
