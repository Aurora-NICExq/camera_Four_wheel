 ...
 *
 * 全程序所有元素状态转移只存在于本文件 fsm_update() 的唯一 switch 里。
 *
 * ASCII 状态图（Mermaid 版见 README.md）：
 *
 *                       ┌───────────────────────────────────────────┐
 *                       │                 ST_NORMAL                 │
 *                       │ 去抖 M/N + 冷却掩码 + 优先级仲裁          │
 ...
 *                              ▼
 *                       ┌─────────────┐ 双边恢复（迟滞+确认）
 *                       │ ST_RING_EXIT│ ─────────────────► NORMAL(冷却)
 *                       └─────────────┘
 *
 *   ※ 上图画的是正常退出路径；元素状态的超时路径统一进入 ST_RECOVERY，
 * 恢复失败再进入 ST_FAULT 请求解除武装。超时转移在轨迹里记 trigger=-1。 ※ 回
 * NORMAL 启动 COOLDOWN_FRAMES 冷却，屏蔽 COOLDOWN_MASK 里的检测器进入。 ※
 * 失控保护不在本状态机内：它不是元素检测器，永远不被去抖/冷却/掩码影响（见
 * cpu0_main.c）。
 ********************************************************************************************************************/
#include "fsm.h"
#include "config.h"
...
/*-------------------------------------------------------------------------------------------------------------------
 * FSM_STATIC_ASSERT — 编译期断言（C99 可用，不依赖 C11 的 _Static_assert，
 * 故在 -std=c99 -Wpedantic 下无警告；条件为假时把数组长度置负，编译直接失败）。
 * 用途：把 fsm.h 承诺的"契约表与状态枚举同长"从注释变成真正的编译期保证。
 *------------------------------------------------------------------------------------------------------------------*/
#define FSM_STATIC_ASSERT(cond, tag) typedef char fsm_static_assert_##tag[(cond) ? 1 : -1]

/*===================================================================================================================
 * 一、状态契约表（const）—— control.c 只读本表执行，状态机永不直接触碰 PWM
 *==================================================================================================================*/

/* 基础表：环岛三态按【左环】填写（左环＝左侧开口，环内沿外侧=右边线巡线，出环向右偏置驶出）。
 */
/* 数组故意不写死长度（用 []）：长度由下方初始化行数决定，再由文件末尾的
 * FSM_STATIC_ASSERT 核对是否等于 ST_COUNT —— 漏填一行即编译失败。 */
...
/*-------------------------------------------------------------------------------------------------------------------
 * popcount16 — 16 位置位计数（去抖窗口统计用；手写循环避免依赖编译器内建）
 *------------------------------------------------------------------------------------------------------------------*/
...
/*-------------------------------------------------------------------------------------------------------------------
 * det_debounced — 检测器去抖判定：最近 N 帧命中 ≥ M
 * 单帧误检在结构上无法引发转移 —— 这是结构性要求，不是可选优化。
 *------------------------------------------------------------------------------------------------------------------*/
...
/*-------------------------------------------------------------------------------------------------------------------
 * trace_push — 记录一次状态转移（from, to, 帧号, 触发值；超时转移记 -1）
 *------------------------------------------------------------------------------------------------------------------*/
...
/*-------------------------------------------------------------------------------------------------------------------
 * enter_state — 唯一的状态切换出口：登记轨迹、清状态内计数、置 just_entered
 * 回 NORMAL 时启动冷却（元素尾部的噪声边沿最爱立刻二次触发）。
 *------------------------------------------------------------------------------------------------------------------*/
...
/*-------------------------------------------------------------------------------------------------------------------
 * count_lost_rows — 统计行带 [lo, hi) 内某侧丢线行数（环岛入口/出口判定共用）
 *------------------------------------------------------------------------------------------------------------------*/
...
/*-------------------------------------------------------------------------------------------------------------------
 * fsm_update — 每帧唯一入口。执行顺序：
 *   1. 帧计数 / 冷却递减 / just_entered 清零；
 *   2. 去抖窗口更新（被当前状态契约或冷却屏蔽的检测器压 0 ——
 * 屏蔽的是"进入"，窗口也不许积累）；
 *   3. 唯一 switch：本状态的退出判定（迟滞 + 确认帧）与 NORMAL
 * 的进入仲裁（优先级）；
 *   4. 元素状态超时进入恢复态，恢复失败进入故障态。
 *------------------------------------------------------------------------------------------------------------------*/
